...
g = ggplot(mds, aes(x = PC1, y = PC3, colour = train[,class_col]) ) +
  geom_point() + theme_classic()
ggsave("0_all_labs_pc13.png", plot = g, device = "png")
g = ggplot(mds, aes(x = PC2, y = PC3, colour = train[,class_col]) ) +
  geom_point() + theme_classic()
ggsave("0_all_labs_pc23.png", plot = g, device = "png")
...
ggplot() +
  geom_line(aes(x = 1:length( AUC_ROC_random), y = AUC_ROC_random, colour = "random")) +
  geom_line(aes(x = 1:length( AUC_ROC_uncertainty), y = AUC_ROC_uncertainty, colour = "uncertainty")) +
  theme_classic() +
  labs(x = "batch", y = "ROC-AUC", subtitle = "Active ML is a more optimal data search strategy", colour = "Search strategy")
ggsave("overall.png",device = "png")
